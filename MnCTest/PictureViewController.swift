...
class PictureViewController: UIViewController {
    ...
    override func viewDidAppear(_ animated: Bool) {
        DispatchQueue.global(qos: .background).async {
            let URL_IMAGE = URL(string: (Singleton.instance.itemSelected.value(forKey: "urls") as! NSDictionary).value(forKey: "full") as! String)
            ...
            DispatchQueue.main.async {
                self.retrieveData()
                self.textView.text = Singleton.instance.itemSelected.value(forKey: "alt_description") as? String
    ...
    @IBAction func savePicture(_ sender: Any) {
        if !saved {
            ...
            do {
                let jsonData : NSData = try JSONSerialization.data(withJSONObject: Singleton.instance.itemSelected, options: JSONSerialization.WritingOptions.prettyPrinted) as NSData
    ...
    override func viewWillAppear(_ animated: Bool) {
        Singleton.instance.isSaved = false
    }
    func retrieveData() {
        guard let appDelegate = UIApplication.shared.delegate as? AppDelegate else { return }
        let managedContext = appDelegate.persistentContainer.viewContext
        ...
        do {
            ...
            for data in result as! [NSManagedObject] {
                ...
                if saved {
                            ...
                            if dic.value(forKey: "id") as! String == Singleton.instance.itemSelected.value(forKey: "id") as! String {
